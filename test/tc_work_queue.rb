...
class TC_SimpleWorkQueue < Test::Unit::TestCase
  ...
  def test_max_tasks
    assert_raise(ArgumentError) { SimpleWorkQueue.new(nil,0) }
    assert_raise(ArgumentError) { SimpleWorkQueue.new(nil,-1) }
    wq = SimpleWorkQueue.new(1,1)
    wq.enqueue_b { sleep(0.01) }
    wq.enqueue_b { sleep(0.01) }
  ...
  def test_keep_alive
    assert_raise(ArgumentError) { SimpleWorkQueue.new(nil,nil,0) }
    assert_raise(ArgumentError) { SimpleWorkQueue.new(nil,nil,-1) }
    wq = SimpleWorkQueue.new(1,1,0.01)
    wq.enqueue_b { sleep(0.01) }
    assert_equal(wq.cur_threads, 1)
    sleep(0.1)
    assert_equal(wq.cur_threads, 0)
  ...
  def test_stress
    a = []
    m = Mutex.new
    wq = SimpleWorkQueue.new(100,200,0.1)
    (1..1000).each do
